...
export default (state = initialState, action: any) => {
  switch (action.type) {
    ...
    case LOADING_ALL_ADDRESS: {
      return {
        ...state,
        enderecosByCli: mapKeys(action.payload, 'idEndereco'),
    ...
    case ADD_ADDRESS: {
      return {
        ...
        totalPaginas: calcTotalPaginas(
          Object.keys(state.enderecosByCli).length + 1,
        ),
        enderecosByCli: {
          ...state.enderecosByCli,
    ...
    case SET_REMOVE_ADDRESS:
      ...
      return {
        ...
        pagina: novaPagina,
        enderecosByCli: omit(state.enderecosByCli, action.payload),
      };
