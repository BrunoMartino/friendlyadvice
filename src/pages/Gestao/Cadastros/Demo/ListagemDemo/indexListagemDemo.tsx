...
const ListagemOrdensDeServico = () => {
  ...
  const paginacaoRef = useRef<{
    ...
    handleFiltrar: ({ shouldLimit, sqlToExecute }: any) => Promise<any>;
  ...
  const getInfos = useCallback(async () => {
    ...
    try {
      ...
      // const responseCount = await paginacaoRef.current?.handleFiltrar({
      //   sqlToExecute:
      //     getInputSearch.length > 0
      //       ? sqlContadorPaginas(sqlQuery)
      //       : sqlContadorPaginas(new Map().set('situacao', selectedItem?.id)),
      //   shouldLimit: false,
      // });
      ...
      {/* {openFilter && (
        <>
          <FilterSideMenu
            ...
            onSituationChange={(descricao) => {
              // setFilterSideSituationSelected({ descricao })
            }}
